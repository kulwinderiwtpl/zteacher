function DragImgUpload(id,img_url,options) {
    this.me = $(id);
    var defaultOpt = {

    };
    if (img_url.length > 0) {
        this.preview = $('<div id="preview"><img src="' +document.location.protocol+"//"+document.location.host+'' +
            '/'+img_url+'"' +
            ' class="img-responsive"  style="width:100%;height:100%;" alt="" title=""> </div>');
    } else {

        this.preview = $('<div id="preview"><img src="' +document.location.protocol+"//"+document.location.host+
            '/images/headImg000.png"' +
            ' class="img-responsive"  style="width:100%;height:100%;" alt="" title=""> </div>');
    }
...
DragImgUpload.prototype = {
    ...
    onDrop:function (e) {
        ...
        if(fileList[0].type.indexOf('image') === -1){
            alert("You are not dragging a picture!");
            return false;
        }
        ...
        if(filesize>500){
            alert("Upload size cannot exceed 500K.");
            return false;
        }
