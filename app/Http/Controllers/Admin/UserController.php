...
//        $status = DB::transaction(function () use ($request) {
            ...
            if (!ManagerModel::checkPassword($user['username'], $user['oldPassword'])){
                return json_encode('密码输入有误');
            }
            ...
            $salt = \CommonClass::random(4);
            $data = array(
                'password' => ManagerModel::encryptPassword($user['newPassword'], $salt),
                'salt' => $salt,
                'updated_at' => date('Y-m-d H:i:s', time())
            );
            /*if (!ManagerModel::where('id', $id)->where('password', $user['password'])->first()) {
                $salt = \CommonClass::random(4);
                $data = array(
                    'password' => ManagerModel::encryptPassword($user['password'], $salt),
                    'salt' => $salt,
                    'updated_at' => date('Y-m-d H:i:s', time())
                );
            } else {
            ...
            $stu =ManagerModel::where('id', $id)->update($data);
            /*$user = ManagerModel::where('id', $id)->first();
            RoleUserModel::where('user_id', $id)->delete();
            $user->attachRole($request->get('role_id'));*/
